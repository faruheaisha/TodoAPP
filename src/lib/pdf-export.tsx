...
// This file defines the Claude-style PDF template for todo export.
// The actual Document component is defined inline for the PDF renderer.
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <Text style={styles.subtitle}>
          Generated: {new Date().toLocaleDateString(lang === 'zh' ? 'zh-CN' : 'en-US')}
        </Text>
        ...
        {longtermTodos.length > 0 && (
          <View style={{ marginBottom: 20 }}>
            <Text style={styles.sectionTitle}>
              {lang === 'zh' ? '▸ 长时待办' : '▸ Long-term'} ({longtermTodos.length})
            </Text>
            {longtermTodos.map((todo) => (
              <View key={todo.id} style={styles.todoItem}>
                <View style={todo.completed ? styles.todoCheckboxDone : styles.todoCheckbox} />
                <Text style={todo.completed ? styles.todoTextDone : styles.todoText}>
                  {todo.title}
                </Text>
                {todo.deadline && (
        ...
        {quickTodos.length > 0 && (
          <View style={{ marginBottom: 20 }}>
            <Text style={styles.sectionTitle}>
              {lang === 'zh' ? '▸ 临时待办' : '▸ Quick'} ({quickTodos.length})
            </Text>
            {quickTodos.map((todo) => (
              <View key={todo.id} style={styles.todoItem}>
                <View style={todo.completed ? styles.todoCheckboxDone : styles.todoCheckbox} />
        ...
        <View style={styles.footer}>
          <Text style={styles.footerText}>
            {lang === 'zh' ? `共 ${todos.length} 项 · 完成 ${todos.filter((t) => t.completed).length} 项` : `Total: ${todos.length} · Completed: ${todos.filter((t) => t.completed).length}`}
          </Text>
