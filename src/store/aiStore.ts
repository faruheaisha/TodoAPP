...
export interface AIProvider {
  ...
  /** 用户从 fetchedModels 中多选的模型列表 */
  selectedModels: string[];
  /** 当前选中的模型（聊天时实际使用的 model ID） */
  activeModel: string;
  enabled: boolean;
  ...
  lastTestMs?: number;
  lastTestOk?: boolean;
  /** Logo slug（对应 public/logos/providers/ 文件名，不含 .svg） */
  logo?: string | null;
  /** 品牌色 (hex) */
  iconColor?: string | null;
  /** 分类（official / cn_official / aggregator / third_party） */
  category?: string | null;
  /** 预设 ID（对应 PROVIDERS 中的 id，用于找回图标和颜色） */
  ...
  persist(
    (set) => ({
      ...
      updateProvider: (id, patch) =>
        set((s) => {
          ...
          const anyEnabled = providers.some((p) => p.enabled && p.apiKey);
          return { providers, aiEnabled: anyEnabled ? true : s.aiEnabled };
        }),
      ...
      deleteProvider: (id) =>
        set((s) => {
          ...
          const anyEnabled = remaining.some((p) => p.enabled && p.apiKey);
          return {
            providers: remaining,
            activeChatProviderId:
            ...
            aiEnabled: anyEnabled ? true : s.aiEnabled,
            tierModels: {
    ...
    }),
    {
      ...
      migrate: (persisted: unknown, version: number) => {
        ...
        // 确保 providers 数组中的每个元素都有 selectedModels 字段
        const providers = (p.providers as Array<Record<string, unknown>>) ?? [];
        const migrated = providers.map((pr) => {
          if (!pr.selectedModels) {
            const fm = pr.fetchedModels as string[] | undefined;
            const am = pr.activeModel as string | undefined;
            return { ...pr, selectedModels: am && fm?.includes(am) ? [am] : [] };
          }
          return pr;
        });
        return { ...p, providers: migrated };
      },
    }
  )
);
...
/** 按层级获取 AI 配置；层级的 provider 失效时自动 fallback 到 activeChatProvider */
export function getAIConfigByTier(
  tier: keyof TierModelMap
): {
  providerId: string;
  model: string;
  apiKey: string;
  ...
  const entry = state.tierModels[tier];
  if (entry) {
    const [pid, model] = entry.split('::');
    if (pid && model) {
      const cfg = getAIConfigByKey(pid, model);
      if (cfg) return cfg;
    }
  }
  // fallback: 使用当前激活供应商
...
export function getActiveAIConfig(): {
  ...
  const model = p.activeModel || (p.selectedModels && p.selectedModels[0]) || p.fetchedModels[0] || '';
  if (!model) return null;
  return { providerId: p.id, model, apiKey: p.apiKey, baseUrl: p.baseUrl, kind: p.kind };
}

/** 根据 providerId + 模型名 获取 AI 配置（ChatPanel 多模型下拉用） */
export function getAIConfigByKey(providerId: string, model: string): {
  providerId: string;
  model: string;
  apiKey: string;
  baseUrl: string;
  ...
  const { providers, aiEnabled } = useAIStore.getState();
  if (!aiEnabled) return null;
  const p = providers.find((x) => x.id === providerId);
  if (!p?.apiKey || !p.enabled || !model) return null;
  return { providerId: p.id, model, apiKey: p.apiKey, baseUrl: p.baseUrl, kind: p.kind };
}

/** 已配 key 的供应商列表（每个 selectedModel 一条记录，ChatPanel 模型切换下拉用） */
